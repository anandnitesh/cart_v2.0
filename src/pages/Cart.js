...
const Cart = () => {
    ...
    const getSum = (productId, price) => {
        ...
        total += sum; // to get the total amount for all items.
        return sum;
    }
    ...
    return (
        !products.length ?
        <img  className = "mx-auto w-1/3 mt-12" src = "/images/empty-cart.png" alt = ""></img>
                    ...
                    products.map(product => {
                        return (
                                ...
                                <div className = "flex items-center justify-between">
                                    ...
                                    <div>
                                        <button onClick = {() =>{decrement(product._id)}} className = "bg-yellow-500 px-4 py-2 rounded-full leading-none font-bold">-</button>
                                        <b className ="px-4">{getQty(product._id)}</b>
                                        <button onClick = {() =>{increment(product._id)}} className = "bg-yellow-500 px-4 py-2 rounded-full leading-none font-bold">+</button>
                                    </div>
